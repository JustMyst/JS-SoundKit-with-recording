...
// lista dŸwiêków przypisana do ka¿dego klawisza
let soundList = {
    a: "boom",
    s: "clap",
    d: "hihat",
...
function loadAudio() { //wczytanie dŸwiêków
...
function startRecord(numer_sciezki) { // nagrywanie sciezki
    if (!record[numer_sciezki]) { // jesli nie ma sciezki o danym numerze, tworzy j¹ z aktualnym czasem, zaczyna nagrywaæ i zmienia napis na przycisku
    ...
    else { // jesli istnieje sciezka, to przestaje j¹ nagrywaæ i wyswietla informacje o sciezce.
...
function playKey(e) { // funkcja grania muzyki
    ...
    for (i = 1; i <= sciezka.length; i++){ // sprawdza, czy w³¹czone jest nagrywanie dla ka¿dej œcie¿ki
        if (record[i]) {
            if (Object.keys(soundList).indexOf(e.key) > -1) { //jeœli naciskany jest poprawny klawisz, dodaje go do sciezki z czasem nagrania
                sciezka[i].push(
                    {
...
function playRecord(numer_sciezki) { //odtwarzanie sciezki
    if (!timeout[numer_sciezki] && sciezka[numer_sciezki][0]) { // jeœli nie ma timeout dla sciezki, tworzy go dla ka¿dego elementu sciezki z odpowiadaj¹cym mu opóŸnieniem.
        ...
        koniec[numer_sciezki] = setTimeout(() => playRecord(numer_sciezki), sciezka[numer_sciezki][sciezka[numer_sciezki].length - 1].delay + 10) // gdy skoñczy siê sciezka, przycisk automatycznie zmienia siê ponownie na "play"
    }
    else { // jeœli naciœniêty bêdzie przycisk podczas grania sciezki, usuwa siê wszystkie timeouty i przywraca przycisk do "Play"
